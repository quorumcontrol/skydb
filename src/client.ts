...
import { defaultPortalUrl, addUrlQuery, makeUrl } from "./utils";
...
export class SkyDb {
  ...
  getFileContent = getFileContent
    ...
    // SkyDB helpers
    registry = {
      getEntry: getEntry.bind(this) as OmitThisParameter<typeof getEntry>,
      getEntryUrl: getEntryUrl.bind(this) as OmitThisParameter<typeof getEntryUrl>,
      setEntry: setEntry.bind(this) as OmitThisParameter<typeof setEntry>,
    };

      // SkyDB
  db = {
    getJSON: getJSON.bind(this) as OmitThisParameter<typeof getJSON>,
    setJSON: setJSON.bind(this) as OmitThisParameter<typeof setJSON>,
  };
   ...
   */
  constructor(
    portalUrl: string = defaultPortalUrl(),
    customOptions: CustomClientOptions = {}
  ) {
  ...
  executeRequest(config: RequestConfig): Promise<AxiosResponse> {
    if (config.skykeyName || config.skykeyId) {
      throw new Error(
        "Unimplemented: skykeys have not been implemented in this SDK"
      );
    }
    ...
    let formHeaders:FormData.Headers = {}
    if (config.data?.getHeaders) {
      formHeaders = (config.data as FormData).getHeaders()
    }

    // Build headers.
    const headers = { ...config.headers, ...formHeaders };
